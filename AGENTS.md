...
- **Then grill, always → understand before spec'ing**: run `/grill` on the gathered input — idea *or* written ticket — before writing anything. It interrogates in small question batches (assuming it doesn't yet understand), surfacing ambiguities, edge cases, and contradictions, and resolving open questions *up front*. The ticket is input, not final truth. Output is shared understanding that feeds design routing and sharpens the spec; trivial fixes can skip it. See "Grill" (stage 0.5) in `docs/harness.md`.
- **Meaningful change → one spec first**: copy `docs/specs/_template.md` to `docs/specs/YYYY-MM-DD-fra-xxx-<slug>.md` and fill it (Context section = the grill's resolved understanding). Trivial fixes don't need one.
